...
# Keep strict for Pre-Alpha so old or mismatched files do not silently reconstruct
# against the wrong packet specification.
REQUIRE_HEADER_METADATA = True
...
def parse_emeimg_header(packet, line_number=None):
    ...
    if len(packet) != HEADER_LENGTH:
        raise ValueError(
            f"Invalid EMEIMG metadata header{location}: expected {HEADER_LENGTH} chars, got {len(packet)}: {repr(packet)}"
        )

    if not packet.startswith(HEADER_IDENTIFIER):
        raise ValueError(
            f"Invalid EMEIMG metadata header{location}: missing {HEADER_IDENTIFIER} identifier: {repr(packet)}"
        )
...
def parse_metadata_comment(line, line_number=None):
    stripped = line.strip()
    ...
    if key in {"EMEIMG_VERSION", "VERSION", "VV"}:
        metadata["version"] = normalize_version(value)
        return metadata

    if key in {"EMEIMG_PATCH", "PATCH", "P"}:
...
def merge_metadata(existing, incoming):
    ...
    for key in ("version", "patch", "experimental"):
        ...
        if key in existing and existing[key] != incoming[key]:
            raise ValueError(
                f"Conflicting .emeimgout metadata for {key}: {existing[key]} vs {incoming[key]}"
            )
...
def validate_metadata_compatibility(metadata, filename):
...
def load_commands_from_file(filename):
    ...
    with open(filename, "r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            comment_metadata = parse_metadata_comment(line, line_number)
            metadata = merge_metadata(metadata, comment_metadata)
    ...
    print(f"Loaded {len(commands)} drawable packet lines from {filename}")

    if metadata is None:
        print("No EMEIMG metadata header found.")
    else:
        print(f"Loaded metadata from {metadata.get('source', 'metadata')}.")
...
def main():
    ...
    if not validate_metadata_compatibility(metadata, input_filename):
        return

    if not commands:
        print("No drawable commands loaded; reconstruction cancelled.")
        return
