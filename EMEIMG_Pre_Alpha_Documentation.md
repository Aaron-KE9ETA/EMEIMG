...
## Constructor - Pre-Alpha Status

The EMEIMG Constructor has now been added to the Pre-Alpha development branch as the image design and packet export stage of the EMEIMG pipeline.

The Constructor provides a GUI-based packet-first vector editor for building simple EMEIMG images as ordered JT65B-compatible drawing commands. Users can select colors, shapes, macro elements, and command parameters, then place elements onto the fixed 720x480 canvas. Each visual element is encoded as a 13-character EMEIMG packet and stored as a layer in the command list.

The Constructor exports `.emeimg` command files for later processing by the Feeder. These files include hard-coded EMEIMG header metadata generated by the program itself rather than user-adjustable GUI fields. This keeps version/header handling consistent with the Pre-Alpha pipeline while avoiding unnecessary controls in the interface.

### Constructor UI Updates
...
- Hard-coded section headers are used to organize the interface.
- Header metadata is handled internally by the code and is not adjustable through the GUI.
...
### Current Constructor Role in the Pipeline
...
4. Marking critical/priority elements when needed.
5. Exporting the resulting `.emeimg` command file.
6. Supplying the input file used by the Feeder stage.

The Constructor does not directly transmit packets and does not interface with WSJT-X. Its role is limited to image creation and `.emeimg` packet file generation.
...
1. Build Image in Constructor
    ...
    Current status:
        The Constructor has now been added to the Pre-Alpha branch and serves as the GUI-based image design and `.emeimg` export stage of the pipeline.
